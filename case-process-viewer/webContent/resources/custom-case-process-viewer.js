const DIAGRAM_IFRAME_ID = "case-process-viewer";
...
const HIDDEN_IMAGE_ID = "process-url-hidden-image";
...
const PROCESS_URL_PARAM = "&processUrl=";
...
function updateUrlForIframe() {
  ...
  let url;
  if (currentViewerUrl.includes(PROCESS_URL_PARAM)) {
    // If processUrl param already exists, replace it
    url = currentViewerUrl.replace(PROCESS_URL_PARAM, PROCESS_URL_PARAM + encodedDataUrl);
  } else {
    // If processUrl param doesn't exist, add it
    url = currentViewerUrl + PROCESS_URL_PARAM + encodedDataUrl;
  }
...
async function captureScreenFromIframe() {
  ...
  const iframe = document.querySelector("[id$='case-process-viewer']");
...
function openViewerInNewTab() {
  var url = $("[id$='case-process-viewer']").prop("src");
  if (url) {
